...
@pytest.mark.asyncio
async def test_execute_move_tower_of_hanoi(client):
    ...
    move_result = await call_tool(client, "execute_move", {"env_id": env_id, "move": [1, 0, 2]})

    assert move_result["move_successful"] is True
    assert isinstance(move_result["state"], list)  # State should be a tuple of lists
    assert len(move_result["state"]) == 3  # 3 pegs

    # The smallest disk (1) should now be on peg 2
    assert 1 in move_result["state"][2]


@pytest.mark.asyncio
async def test_execute_move_river_crossing(client):
    ...
    move_result = await call_tool(
        client, "execute_move", {"env_id": env_id, "move": ["a_1", "A_1"]}
    )

    assert move_result["move_successful"] is True
    assert isinstance(move_result["state"], list) or isinstance(move_result["state"], tuple)
    # Check that boat position changed to 1 (right side)
    assert move_result["state"][0] == 1
    # Check that actor a_1 and agent A_1 are now on the right side (1)
    entity_positions = move_result["state"][1]
    assert entity_positions["a_1"] == 1
    assert entity_positions["A_1"] == 1


@pytest.mark.asyncio
async def test_execute_move_validation(client):
    ...
    result = await call_tool(client, "execute_move", {"env_id": "invalid_id", "move": [1, 0, 2]})
    assert "error" in result
    ...
    result = await call_tool(
        client,
        "execute_move",
        {"env_id": env_id, "move": [1, 1, 2]},  # Peg 1 is empty at the start
    )
    assert result["move_successful"] is False
...
async def test_reset_simulator(client):
    ...
    # Make a move
    await call_tool(client, "execute_move", {"env_id": env_id, "move": [1, 0, 2]})
