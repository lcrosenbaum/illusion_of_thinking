...
class SimulationEnvironment:
    ...
    def __init__(self, simulator_type: str, simulator_params: Dict[str, Any]):
        ...
        self.simulator = self._create_simulator(simulator_type, simulator_params)
        self.id = str(uuid.uuid4())
        self.last_accessed = time.time()  # Add timestamp for tracking last access

    def _create_simulator(self, simulator_type: str, params: Dict[str, Any]) -> Simulator:
        """Create a simulator instance based on the specified type and parameters."""
...
def init_simulator(simulator_type: str, N: int, k: Optional[int] = None) -> Dict[str, Any]:
    ...
    """
    if simulator_type not in ["TowerOfHanoi", "RiverCrossing"]:
        return {"error": "Invalid simulator type. Must be 'TowerOfHanoi' or 'RiverCrossing'"}
...
@mcp.tool
def execute_move(env_id: str, move: Union[List[int], List[str]]) -> Dict[str, Any]:
    """
    ...
    if environment.simulator_type == "TowerOfHanoi" and isinstance(move, list) and len(move) == 3:
        # Convert to tuple for TowerOfHanoi
        move_tuple = tuple(move)
        move_was_successful = environment.simulator.execute_move(move_tuple)
    else:
        move_was_successful = environment.simulator.execute_move(move)

    state = environment.simulator.state
    goal_reached = environment.simulator.is_goal_reached()

    return {"move_successful": move_was_successful, "state": state, "goal_reached": goal_reached}
